...
var _connected_signals: Array = [] # [{obj, signal_name, callable}] for cleanup
...
func _cleanup_parameters() -> void:
	# Disconnect all tracked signals before clearing
...
func _connect_parameter_change(control: Control, _param_name: String) -> void:
	# Connect appropriate signal on the actual input widget, not the container.
	# UIBuilder returns containers (VBoxContainer) for most types —
	# the actual input (SpinBox, Slider, LineEdit, etc.) is nested inside.

	if control is CheckBox:
		_track_signal(control, "toggled", func(_v): _on_parameter_changed())
		return

	if control is VBoxContainer:
		for child in control.get_children():
			if child is SpinBox:
				_track_signal(child, "value_changed", func(_v): _on_parameter_changed())
				return
			elif child is LineEdit:
				_track_signal(child, "text_changed", func(_t): _on_parameter_changed())
				return
			elif child is OptionButton:
				_track_signal(child, "item_selected", func(_i): _on_parameter_changed())
				return
			elif child is ColorPickerButton:
				_track_signal(child, "color_changed", func(_c): _on_parameter_changed())
				return
			elif child is HBoxContainer:
				# Float control (Slider + SpinBox) or Vector control (SpinBoxes)
				for grandchild in child.get_children():
					if grandchild is Slider:
						_track_signal(grandchild, "value_changed", func(_v): _on_parameter_changed())
						return
				# Vector: connect all SpinBoxes
				for grandchild in child.get_children():
