...
Context: Solido Tri‑D is a Godot 4 shader-development platform with a schema-driven editor plugin that parses TOML and builds Control-based UI, so connectors must be data-driven and reproducible. [cite:180]
...
### Node
A UI module: panel, strip, inspector section, stack group, viewport overlay, etc.

### Port
A connector attachment point on a Node. Ports live on the Node boundary (or a small offset from it) and have direction + style constraints.

### Net
A logical connection between ports, representing relationship: data flow, dependency, grouping, or selection linkage.

### Segment
A piece of a connector path. A Net can be: direct, bus+branch, or multi-drop.

### Bus
A shared backbone rail line (often orthogonal) used by many Nets, with branch arcs to endpoints.

### Reserved zones
Rectangles where connectors should not pass (text fields, dense UI, critical thumbnails).
...
AUTO = place using heuristic (best spacing on that side),

PARAM = use pos_u (0..1) along side,

ABS = use offset_px from a reference corner
...
3.1 High-level flow
Collect geometry: node rects, reserved zones, port candidates.

Resolve port positions (AUTO → concrete points):

Partition each side into lanes (based on u, thickness, and requested spacing).

Sort ports by priority desc, then node_id, then port_id (tie-breaker).
...
3.2 Clearance & lanes
Define a global clearance_px = max(1u, 1.25 * thickness_px).
...
Use a fixed, bounded set of candidate detours:

lane_shift: move branch attach point up/down (or left/right) by N lanes

bus_attach_alt: attach to a different bus lane

control_point_push: push Bézier control points away from obstacles by fixed increments

style_fallback: ARC → ELBOW for that net only (last resort)

Always pick the first candidate that satisfies constraints in a stable, ordered list.

4) Spline construction spec
4.1 Curve choice
Use cubic Bézier segments:

Start at P0 (from-port)
...
t = clamp(d * 0.25, min_t, max_t) where min_t = 3u, max_t = 16u

dir0 = outward normal from port side

dir3 = outward normal from port side (but pointing backward into approach)
...
If the curve would exceed max_curvature (approx by sampling), reduce t or use two-stage routing via a bus anchor.

4.3 Bus+branch
A BUS_BRANCH net becomes:

ARC: from-port → bus entry point

BUS: along bus lane (orthogonal rail)

ARC: bus exit point → to-port

This keeps the “LCARS rails” aesthetic and makes routing stable.

4.4 Sampling for rendering
Sample curve at N points (N based on length; e.g., 16–64).
...
A thick polyline mesh (recommended), OR

A shader-based ribbon quad strip.
...
Hover on a connected module:

highlight all attached nets (alpha + rim boost)

Active selection:

primary net pulses slightly (0.1–0.2 amplitude)

endpoints show “target brackets” (optional)
...
Apply a mild “contrast guard”:

If sampled background luminance is too close to connector luminance, raise rim contrast (not saturation).

7) Implementation plan (Godot / Solido)
7.1 Suggested files
scripts/ui/connectors/port.gd

scripts/ui/connectors/net.gd

scripts/ui/connectors/router.gd

scenes/ui/connectors/SplineConnector.tscn

scenes/ui/connectors/BusRail.tscn

shaders/ui/connector_ribbon.gdshader

shaders/ui/glass_pane.gdshader

7.2 Integration points
UI builder emits Nodes with node_id and port definitions.
...
Sort everything with explicit tie-breakers (IDs).

Quantize key values to u-based steps (lane offsets, bus lane spacing).

Avoid float drift: round sampled points to 0.25 px (or similar) before mesh build.

Keep routing candidates ordered and bounded.
  ...
  "nodes": [
    {
      ...
      "rect":,[2][1]
      "ports": [
        {"port_id":"stack.active", "side":"E", "pos_mode":"PARAM", "pos_u":0.2, "role":"BLEND", "priority":90, "style":"ARC", "thickness_u":2.5, "cap":"ENDCAP"}
      ],
      "reserved_zones": [][3]
    },
    {
      ...
      "rect":,[1]
      "ports": [
        {"port_id":"inspector.focus", "side":"W", "pos_mode":"PARAM", "pos_u":0.35, "role":"BLEND", "priority":90, "style":"ARC", "thickness_u":2.5, "cap":"BRACKET"}
      ],
      "reserved_zones": []
    }
  ],
  "buses": [
    {"bus_id":"right_bus", "orientation":"VERT", "lane_count":3, "rect":, "role":"NAV", "thickness_u":2.0}[2][1]
  ],
  "nets": [
    {"net_id":"active_to_focus", "kind":"HIGHLIGHT", "importance":90, "from":["stack.active"], "to":["inspector.focus"], "routing":{"mode":"BUS_BRANCH","bus_id":"right_bus"}}
  ]