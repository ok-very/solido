// Re-export parent module items so inner test modules can use `use super::*;`
use super::*;
...
mod tests {
    ...
    fn mod_adds_to_base() {
        ...
        let (mut org, handles) = OrganismDsp::from_dna(&dna, SR, None).expect("build failed");
    ...
    fn mod_gain_scales_depth() {
        ...
        let (mut org_small, handles_small) = OrganismDsp::from_dna(&dna_small, SR, None).unwrap();
        ...
        let (mut org_large, handles_large) = OrganismDsp::from_dna(&dna_large, SR, None).unwrap();
    ...
    fn all_species_load_dna() {
        ...
        for &(path, species, expected_cells, expected_wires) in cases {
            ...
            let result = OrganismDsp::from_dna(&dna, SR, None);
...
mod click_diagnostic {
    ...
    fn click_detector_all_organisms() {
        ...
        for path in &dna_files {
            ...
            let (mut org, _handles) = OrganismDsp::from_dna(&dna, SR, None)
